...
const DesempregadoModal = ({ selected, setOpen, user }) => {
    return (
        <Modal>
            ...
            <div className="p-6 overflow-y-scroll h-full space-y-10">
                ...
                <div className="absolute top-0 left-0 w-full bg-white p-6 border-b flex justify-between items-center">
                    <div className="w-10/12 flex md:justify-between items-center">
                        <h2 className="text-3xl font-bold">{selected.nome}</h2>
                        {user?.atuacao === "RH" && (
                            <button
                                onClick={() => alert("Mensagem enviada")}
                                className="w-full md:w-fit md:px-4 px-2 py-2 max-md:h-fit cursor-pointer bg-blue-600 hover:bg-blue-500 text-white text-sm md:font-semibold md:uppercase rounded-md transition-all"
                            >
                                Enviar mensagem
                            </button>
                        )}
